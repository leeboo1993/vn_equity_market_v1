...
function getPriceAtDate(ticker, dateStr, historyMap) {
    ...
    // Find matching date or closest previous
    // History is sorted asc by time ("YYYY-MM-DD")
    let best = null;
    for (let i = 0; i < history.length; i++) {
        // history[i].time should be "YYYY-MM-DD"
        if (history[i].time > targetDateStr) break;
        best = history[i];
    }
...
function enrichReport(report, historyMap, stockInfoMap) {
    ...
    if (ticker && stockInfoMap.has(ticker)) {
        ...
        // Normalize existing values - convert "undefined" string or "NA" to null
        const normalize = (val) => {
        ...
        const currentSector = normalize(report.info_of_report.sector);
        const currentName = normalize(report.info_of_report.stock_name);

        // Force CSV data if current is missing or generic "production"
        if ((!currentSector || currentSector === 'production') && info.industry_en) {
            report.info_of_report.sector = info.industry_en;
        }

        if (!currentName && info.short_name) {
            report.info_of_report.stock_name = info.short_name;
        }

        // Final safety for "undefined" labels
        if (report.info_of_report.stock_name === 'undefined') report.info_of_report.stock_name = info.short_name || info.other_name || ticker;
        if (report.info_of_report.sector === 'undefined') report.info_of_report.sector = info.industry_en || '';
    }
...
let cachedLiteReports = null;
let cachedFullReports = null;
let inflightFullDownload = null;

// === FULL Reports (Heavy, 244MB+) ===
export async function getFullRawReports() {
    if (cachedFullReports) return cachedFullReports;
    if (inflightFullDownload) return inflightFullDownload;

    inflightFullDownload = (async () => {
        let reportsRaw = [];
        ...
        try {
            ...
            const fileContent = fs.readFileSync(reportCachePath, 'utf8'); // Can be slow
            ...
            console.error("Error fetching FULL reports:", e);
            return [];
        }
    })();

    try {
        const result = await inflightFullDownload;
        cachedFullReports = result;
        return result;
    } finally {
        inflightFullDownload = null;
    }
}

// === LITE Reports (Fast, ~22MB) ===
export async function getRawReports() {
    if (cachedLiteReports) return cachedLiteReports;

    let reportsRaw = [];
    const reportCachePath = path.join(os.tmpdir(), "company_report_data_lite.json");
    ...
    try {
        ...
        try {
            const headCmd = new HeadObjectCommand({ Bucket: R2_BUCKET, Key: "company_report_data_lite.json" });
            const headRes = await r2Client.send(headCmd);
            const remoteSize = headRes.ContentLength;
            if (localSize > 0 && localSize === remoteSize) needsDownload = false;
        } catch (e) { }
        ...
        if (needsDownload) {
            console.log(`[R2] Downloading company_report_data_lite.json...`);
            const getCmd = new GetObjectCommand({ Bucket: R2_BUCKET, Key: "company_report_data_lite.json" });
            const res = await r2Client.send(getCmd);
            await streamToFile(res.Body, reportCachePath);
        }
        ...
        console.error("Error fetching LITE reports:", e);
        // Fallback to FULL if lite fails/missing
        console.log("Fallback to FULL reports");
    ...
    // 2. Filter invalid tickers (Do this early on raw data)
    reportsRaw = reportsRaw.filter(r => {
        const t = r.info_of_report?.ticker;
        return t && t !== 'NA' && t !== 'na' && t.trim() !== '';
    });

    cachedLiteReports = reportsRaw;
    return reportsRaw;
}

// New Optimized Function: Enrich a batch of reports
export async function enrichReportsBatch(reportsToEnrich) {
    if (!reportsToEnrich || reportsToEnrich.length === 0) return [];

    // 1. Parallel pre-fetch of all dependencies
    const [stockInfoMap, historyMap] = await Promise.all([
        getStockInfoMap(),
        getPriceHistoryMap()
    ]);

    // 2. Sort by date to calculate target price changes
    // (Note: Sorting 200 items is trivial)
    reportsToEnrich.sort((a, b) => {
        const dA = parseIssueDate(a.info_of_report?.date_of_issue);
        const dB = parseIssueDate(b.info_of_report?.date_of_issue);
        return dA - dB;
    });

    // 3. Enrich base metrics
    let reports = reportsToEnrich.map(r => enrichReport(r, historyMap, stockInfoMap));

    return reports;
}

// Main getReports (Backward Compatible - Slow/Heavy)
export async function getReports() {
    if (cachedReports) return cachedReports;

    const reportsRaw = await getRawReports();

    // We need stock info and history for full enrichment
    ...
    // Calculate Target Price Change (Requires sorting by date first)
    reports.sort((a, b) => {
        const dA = parseIssueDate(a.info_of_report?.date_of_issue);
        const dB = parseIssueDate(b.info_of_report?.date_of_issue);
        return dA - dB;
...
export async function getReport(id) {
    // For single report detail, we need the FULL data (forecast tables, etc.)
    // We cannot use getReports() because that now uses LITE data.
    const fullReports = await getFullRawReports();

    // We also need to ENRICH it
    const reportRaw = fullReports.find(r => String(r.id) === String(id));
    ...
    return enrichReport(reportRaw, historyMap, stockInfoMap);
}
