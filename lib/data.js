...
export async function getSSIAccessToken() {
    ...
    let consumerID = process.env.SSI_CONSUMER_ID;
    let consumerSecret = process.env.SSI_CONSUMER_SECRET;

    if (!consumerID || !consumerSecret) {
        // Fallback to directly fetching from .env file directly if process.env fails
        try {
            const envPath = path.resolve(process.cwd(), '.env');
            if (fs.existsSync(envPath)) {
                const envContent = fs.readFileSync(envPath, 'utf8');
                const envVars = dotenv.parse(envContent);
                consumerID = consumerID || envVars.SSI_CONSUMER_ID;
                consumerSecret = consumerSecret || envVars.SSI_CONSUMER_SECRET;
            }
        } catch (e) { /* ignore */ }
    }

    if (!consumerID || !consumerSecret) {
        console.error("Missing SSI credentials in environment for fallback to R2");
        return null;
    }
