...
function enrichReport(report, historyMap, stockInfoMap) {
    ...
    if (hist && hist.dates.length > 0) {
        ...
        if (rec.target_price) {
            // Threshold: 10,000,000 VND (Share prices in Vietnam rarely exceed 500k-1m, 10m is safe upper bound)
            // 95 billion is definitely an error.
            if (rec.target_price > 10000000) {
...
export async function getReports() {
    ...
    reports = reports.filter(r => {
        ...
        if (!t) return false;
        if (t === 'NA') return false; // Basic cleanup for explicit NA if desired, but user said "do not need to filter"
        // actually user said "we do not need to filter it", so let's be permissive.
        // But 'NA' usually means 'No Available', so technically it is a ticker string "NA". 
        // If the user says "it default to click to has target price", they imply the UI filters "useful" ones.
        // I will just revert to the previous state: filter if no ticker.
