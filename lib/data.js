...
function getPriceAtDate(ticker, dateStr, historyMap) {
    ...
    const targetDate = new Date(2000 + yy, mm - 1, dd); // Month is 0-indexed

    // Find matching date or closest previous
    // History is sorted asc
    let best = null;
    for (let i = 0; i < history.length; i++) {
        const itemDate = new Date(history[i].time);
        if (itemDate > targetDate) break; // Passed the date
        best = history[i];
    }
...
function enrichReport(report, historyMap, stockInfoMap) {
    ...
    if (currentPrice && report.recommendation) {
        ...
        // Performance for 1M, 3M, 6M, 1Y (Relative to NOW)
        // Dashboard expects: return_1m, return_3m, return_6m, return_1y
        const safePerf = (pastPrice) => {
            if (pastPrice && pastPrice > 0) {
                return parseFloat((((currentPrice - pastPrice) / pastPrice) * 100).toFixed(1));
            }
            return null;
        };

        const today = new Date();
        const getPastDateStr = (months) => {
            ...
            const day = d.getDate();
            return `${y < 10 ? '0' + y : y}${m < 10 ? '0' + m : m}${day < 10 ? '0' + day : day}`;
        };

        const p1m = getPriceAtDate(ticker, getPastDateStr(1), historyMap);
        const p3m = getPriceAtDate(ticker, getPastDateStr(3), historyMap);
        const p6m = getPriceAtDate(ticker, getPastDateStr(6), historyMap);
        const p1y = getPriceAtDate(ticker, getPastDateStr(12), historyMap);

        report.recommendation.return_1m = safePerf(p1m);
        report.recommendation.return_3m = safePerf(p3m);
        report.recommendation.return_6m = safePerf(p6m);
        report.recommendation.return_1y = safePerf(p1y);
    }
...
export async function getTickerPrices(ticker) {
    if (!ticker) return [];

    // Ensure history map is loaded
    const historyMap = await getPriceHistoryMap();

    if (historyMap.has(ticker)) {
        return historyMap.get(ticker).map(p => ({
            time: p.time, // YYYY-MM-DD
            value: p.close
