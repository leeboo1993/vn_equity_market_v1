...
import { getStockInfoMap } from './stockInfo'; // Import stock info
import { getPriceHistoryMap } from './priceHistory'; // Import price history helper
...
function enrichReport(report, historyMap, stockInfoMap) {
    ...
    if (ticker && stockInfoMap.has(ticker)) {
        ...
        // Ensure company name exists (map uses 'stock_name' or 'short_name')
        if (!report.info_of_report.stock_name && info.other_name) {
            report.info_of_report.stock_name = info.other_name; // Use other_name from mapping as stock name
    ...
    if (ticker && historyMap.has(ticker)) {
        ...
        // Get latest price (last item)
        if (history.length > 0) {
            const last = history[history.length - 1]; // sorted by date asc in getPriceHistoryMap?
            // Wait, getPriceHistoryMap returns map of arrays. Let's assume they are sorted or use latest.
            // Actually getPriceHistoryMap implementation:
            // It reads parquet. We need to verify sort order. Assuming it's time series.
