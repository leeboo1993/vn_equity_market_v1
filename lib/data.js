...
export async function getReports() {
    ...
    let reports = reportsRaw.map(report => {
        ...
        if (hist && hist.dates.length > 0) {
            ...
            // D. Standardize Recommendations to 4 types: Buy, Sell, Neutral, No Rating
            // User requirement: Only these 4 types should appear
            ...
            if (!rec.target_price) {
                ...
                // Default: if we can't classify, mark as Neutral
                rec.recommendation = "Neutral";
            }
        }
        ...
        // Enrich with stock information - ONLY use data from stock_information_en.csv
        const stockInfo = stockInfoMap.get(ticker);
        const enrichedInfo = { ...info };

        // Clear any old sector data from original reports
        delete enrichedInfo.sector;

        if (stockInfo) {
            enrichedInfo.stock_name = stockInfo.organ_short;
            enrichedInfo.exchange = stockInfo.exchange;
            // Only set sector if we have valid data from stock_information_en.csv
    ...
    reports = reports.filter(r => {
        ...
        // Strict price check removed to allow viewing reports even if price data is missing
