...
// SSI API Token Caching
let ssiAccessTokenStr = null;
let tokenExpiresAt = 0;

export async function getSSIAccessToken() {
    ...
    try {
        ...
        if (!res.ok) {
            console.error(`Failed to authenticate with SSI: ${res.statusText}`);
            return null;
        }
        ...
        if (data.status === 200 && data.data) {
            ...
            console.error(`SSI Auth failed: ${data.message || 'Unknown error'}`);
            return null;
        }
    } catch (e) {
        console.error("SSI API Error:", e);
        return null;
    }
}
...
export function enrichReport(report, historyMap, stockInfoMap) {
    ...
    if (currentPrice && report.recommendation) {
        ...
        if (tp > 1500000) {
            ...
            // Upside Now (Current vs Target) - purely for display
            ...
            // 1. Calculate Upside At Call (Historical Context) logic FIRST
            if (priceAtCall && priceAtCall > 0) {
                ...
                // Recommendation based on Price At Call (Historical Accuracy)
                // If we have price at call, THIS is the truth for "Buy/Sell" at that moment
                if (upAtCall > 15) {
                ...
                // Fallback: If we don't have price at call, maybe use current upside? 
                // Or keep original recommendation? Let's use current upside as a proxy if history missing.
                if (upside > 15) {
                    report.recommendation.recommendation = 'Buy';
                } else if (upside < -5) {
                    report.recommendation.recommendation = 'Sell';
...
let cachedThinReports = null;
let lastThinFetchTimestamp = 0;

export async function getReportsMetadata() {
    // Compute dynamic metadata to ensure fresh quarters and tickers
...
export async function getThinReports() {
    ...
    // Create new thin cached version if raw was updated or thin cache missing
    if (cachedThinReports && (now - lastThinFetchTimestamp < RAW_CACHE_TTL) && cachedThinReports.length === rawReports.length) {
         return cachedThinReports;
    }

    const tpMap = new Map();
    const thinList = rawReports.map(r => {
        const info = r.info_of_report || {};
        const rec = r.recommendation || {};

        // Calculate Target Price Change relative
        let tpChange = null;
    ...
    cachedThinReports = thinList;
    lastThinFetchTimestamp = now;
    return thinList;
}
...
export async function enrichReportsBatch(reportsRaw) {
    ...
    const reports = enriched.filter(r => {
        ...
        // Filter out if no ticker OR if ticker is not exactly 3 alphanumeric characters
        if (!t || typeof t !== 'string') return false;

        const cleanTicker = t.trim();
        // Regex: Exactly 3 characters, A-Z or 0-9. Case insensitive.
        if (!/^[a-zA-Z0-9]{3}$/.test(cleanTicker)) return false;
...
export async function getTickerPrices(tickers, callDates) {
    ...
    // 1. Prepare to fetch live prices from SSI
    ...
    if (token && tickers.length > 0) {
        ...
        const BATCH_SIZE = 10;
        const uniqueTickers = [...new Set(tickers)]; // Don't fetch same ticker multiple times
            ...
            const promises = batch.map(async (ticker) => {
                ...
                try {
                    const priceRes = await fetch(url, {
                        ...
                        // Next.js caching: Next.js fetch cache for performance
                        next: { revalidate: 30 }
                    });
                    ...
                    if (priceRes.ok) {
                        ...
                        if (pData.data && pData.data.length > 0) {
                            ...
                            livePricesMap[ticker] = parseFloat(latestRecord.ClosePrice || latestRecord.Close || 0);
                        }
                    }
                } catch (e) {
                    console.error(`Failed to fetch live price for ${ticker} from SSI. Will fallback to R2.`, e);
                }
            });
    ...
    // 2. Build final result combining live prices (primary) and R2 (fallback) for current price
    for (let i = 0; i < tickers.length; i++) {
        ...
        // Current Price (SSI Live API -> R2 Fallback)
        let priceNow = 0;
        if (livePricesMap[ticker] && livePricesMap[ticker] > 0) {
            // SSI provides live price
            priceNow = livePricesMap[ticker];
        } else if (historyMap.has(ticker)) {
            // Fallback to R2
        ...
        // Price at Call (Always from Historical Parquet in R2)
