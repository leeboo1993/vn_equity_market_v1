...
export async function enrichReportsBatch(reportsToEnrich) {
    ...
    // Cleanup & Calculate Target Price Change (Logic moved here or kept on full set? 
    // Target price change needs PREVIOUS report. This is tricky for pagination.
    // However, Dashboard only shows change if available. 
    // Calculating "change vs previous" requires finding the previous report in the FULL list.
    // For specific dashboard optimization, we might skip "Target Price Change" calculation for paginated views OR
    // we accept we only calculate it against reports IN THE BATCH (which is wrong).
    // OR we load Raw Reports (fast), sort them, finding the previous report is fast (metadata only).

    // Let's keep specific "Target Price Change" logic simple:
    // If we want perfect target price change, we need to know previous.
    // For now, let's just enrich with price/performance which is the heavy part.
    // Re-implementing "Target Price Change" fully efficiently requires more work (scanning history).
    // Given the user wants SPEED, stripping "Target Price Change" accuracy slightly or doing it lazily is better.
    // Let's implement the basic enrichment first.
