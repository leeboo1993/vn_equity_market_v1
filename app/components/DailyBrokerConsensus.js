...
export default function DailyTrackingPage({ hideHeader = false }) {
    ...
    const sentimentHeatmap = useMemo(() => {
        ...
        const activeBrokers = uniqueBrokers.filter(broker => {
            return dates.some(date =>
                allReports.some(r => {
                    if (r.broker !== broker) return false;
                    const rDate = r.info_of_report?.date_of_issue;
                    return rDate === date || (rDate && date && formatDateDisplay(rDate) === formatDateDisplay(date));
                })
            );
        });
        ...
        activeBrokers.forEach(broker => {
            ...
            dates.forEach((date, i) => {
                // Try exact match first, then fallback to normalized comparison
    ...
    return (
        <div style={hideHeader ? { paddingBottom: '2rem' } : {}}>
            {!hideHeader && <Header title="Daily Tracking" />}
            ...
            <main className="daily-tracking-grid">
                ...
                <div className="daily-right-column">
                    ...
                    <section className="card daily-card" style={{ flex: '0 0 auto', height: '320px', display: 'flex', flexDirection: 'column' }}>
                        <div className="daily-card-header">
                            <h3 className="daily-card-title" style={{ color: 'var(--accent)' }}>Market Sentiment</h3>
                        </div>
                        <div className="heatmap-container" style={{ flex: 1, overflow: 'auto', position: 'relative' }}>
                            {isLoading ? (
                                <div className="placeholder-text">Loading...</div>
                            ) : (
                                <>
                                    <table className="heatmap-table" style={{ borderCollapse: 'separate', borderSpacing: 0 }}>
                                        <thead style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#0a0a0a' }}>
                                            <tr>
                                                <th style={{ textAlign: 'left', paddingLeft: '8px', position: 'sticky', left: 0, zIndex: 11, backgroundColor: '#0a0a0a' }}>Broker</th>
                                                {sentimentHeatmap.dates.length > 0 ? sentimentHeatmap.dates.map(d => (
                                                    <th key={d}>{formatDateDisplay(d).slice(0, 5)}</th>
                                                )) : <th>No Data</th>}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {sentimentHeatmap.brokers.length > 0 ? sentimentHeatmap.brokers.map(broker => (
                                                <tr key={broker}>
                                                    <td className="broker-cell" style={{ position: 'sticky', left: 0, zIndex: 5, backgroundColor: '#0a0a0a' }}>{broker.toUpperCase()}</td>
                                                ...
                                                <tr>
                                                    <td colSpan={sentimentHeatmap.dates.length + 1} style={{ textAlign: 'center', padding: '1rem', color: '#666' }}>
                                            ...
                                            {/* Summary Row */}
                                            <tr className="summary-row" style={{ backgroundColor: '#0a0a0a', borderTop: '2px solid #333' }}>
                                            ...
                                            <tr className="comparison-row" style={{ backgroundColor: '#0a0a0a', borderTop: '1px solid #333' }}>
                                                <td className="broker-cell" style={{ color: '#fff', borderRight: '1px solid rgba(255,255,255,0.1)', lineHeight: '1.2' }}>
                                                    VNI (+1)
                                                </td>
                                                {sentimentHeatmap.dates.map(d => {
                    ...
                    <section className="card daily-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '400px' }}>
                        <div className="daily-card-header">
                        ...
                        <div className="rec-table-wrapper" style={{ flex: 1, overflowY: 'auto' }}>
