...
const getCallType = (call) => {
    ...
    if (['buy', 'outperform', 'add', 'accumulate', 'overweight', 'strong buy'].some(k => c.includes(k))) return 'Buy';
...
export default function Dashboard({ reports: propReports, shouldFetchData }) {
    ...
    const [colWidths, setColWidths] = useState({}); // Empty initially for auto-fit

    const resizingRef = useRef(null);

    const handleMouseDown = (e, colKey) => {
        e.preventDefault();
        const th = e.target.closest('th'); // Get the header element safely
        const initialWidth = th ? th.getBoundingClientRect().width : 100;
