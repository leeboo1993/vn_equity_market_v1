...
async function main() {
    ...
    try {
        ...
        for await (const chunk of stream) {
            ...
            if (buffer.includes('"industryLevel": "2"')) {
                const matchIndex = buffer.indexOf('"industryLevel": "2"');
                const start = buffer.lastIndexOf('{', matchIndex);
                if (start === -1) continue;
                ...
                if (end !== -1) {
                    ...
                    const codes = new Set((obj.ratios || []).map(r => r.ratioCode.trim()));
                    console.log(`Industry: ${obj.englishName}`);
                    console.log(`Unique Ratio Codes:`, Array.from(codes).sort());
                    found = true;
                    break;
                }
            }
